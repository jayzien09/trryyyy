...
class NavBarStatesTelemetry extends StatefulWidget {
  const NavBarStatesTelemetry({Key? key}) : super(key: key);

  @override
  _NavBarStatesTelemetryState createState() => _NavBarStatesTelemetryState();
}

class _NavBarStatesTelemetryState extends State<NavBarStatesTelemetry> {
  List<bool> selected = [false, false, true, false, false, false, false];
  void select(int n) {
    for (int i = 0; i < 7; i++) {
      if (i != n) {
        selected[i] = false;
