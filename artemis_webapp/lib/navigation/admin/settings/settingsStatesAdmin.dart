...
class SettingsStateAdmin extends StatefulWidget {
  const SettingsStateAdmin({Key? key}) : super(key: key);

  @override
  _SettingsStateAdminState createState() => _SettingsStateAdminState();
}

class _SettingsStateAdminState extends State<SettingsStateAdmin> {
  List<bool> selected = [false, false, false, false, false, true];
  void select(int n) {
    for (int i = 0; i < 6; i++) {
      if (i != n) {
        selected[i] = false;
