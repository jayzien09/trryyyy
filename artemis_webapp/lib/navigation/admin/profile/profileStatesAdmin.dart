...
class ProfileStateAdmin extends StatefulWidget {
  const ProfileStateAdmin({Key? key}) : super(key: key);

  @override
  _ProfileStateAdminState createState() => _ProfileStateAdminState();
}

class _ProfileStateAdminState extends State<ProfileStateAdmin> {
  List<bool> selected = [false, false, false, false, true, false];
  void select(int n) {
    for (int i = 0; i < 6; i++) {
      if (i != n) {
        selected[i] = false;
