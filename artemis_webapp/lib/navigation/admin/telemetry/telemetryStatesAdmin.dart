...
import '../../../admin/telemetry/telemetryAdmin.dart';

class TelemetryStateAdmin extends StatefulWidget {
  const TelemetryStateAdmin({Key? key}) : super(key: key);

  @override
  _TelemetryStateAdminState createState() => _TelemetryStateAdminState();
}

class _TelemetryStateAdminState extends State<TelemetryStateAdmin> {
  List<bool> selected = [false, false, true, false, false, false];
  void select(int n) {
    for (int i = 0; i < 6; i++) {
      if (i != n) {
        selected[i] = false;
