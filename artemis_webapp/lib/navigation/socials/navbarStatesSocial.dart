...
class NavBarStatesSocials extends StatefulWidget {
  const NavBarStatesSocials({Key? key}) : super(key: key);

  @override
  _NavBarStatesSocialsState createState() => _NavBarStatesSocialsState();
}

class _NavBarStatesSocialsState extends State<NavBarStatesSocials> {
  List<bool> selected = [false, false, false, true, false, false, false];
  void select(int n) {
    for (int i = 0; i < 7; i++) {
      if (i != n) {
        selected[i] = false;
